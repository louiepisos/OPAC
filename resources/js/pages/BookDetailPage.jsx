...
function LargeCover({ isbn, title, format, coverImage }) {
  ...
  const coverUrl = !imgError
    ? (coverImage || (isbn ? `https://covers.openlibrary.org/b/isbn/${isbn}-L.jpg` : null))
    : null;
  ...
  if (coverUrl) {
    return (
      <div style={{
        ...
        background: '#f0ebe0', minHeight: 340, padding: 12,
      }}>
        <img
          src={coverUrl}
          alt={title}
          onError={() => setImgError(true)}
          style={{ width: '100%', height: 340, display: 'block', objectFit: 'contain' }}
        />
      </div>
    );
  }
...
export default function BookDetailPage({ book: initial, onBack }) {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={S.grid}>
        <LargeCover isbn={book.isbn} title={book.title} format={book.format} coverImage={book.cover_image_url} />
        <div>
